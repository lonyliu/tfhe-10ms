...
EXPORT void tfhe_createLweBootstrappingKey(
	...
	LweBootstrappingKey* bk2, 
	LweBootstrappingKey* bk3, 
	LweBootstrappingKey* bk4, 
	const LweKey* key_in, 
	...
	//tGswSymEncryptInt(&bk->bk[i], kin[i], alpha, rgsw_key);
	tGswSymEncryptInt(&bk->bk[i],   kin[i]*kin[i+1]*kin[i+2], alpha, rgsw_key);
	tGswSymEncryptInt(&bk->bk[i+1], (1-kin[i]*kin[i+1]*(kin[i+2]-1)), alpha, rgsw_key);
	tGswSymEncryptInt(&bk2->bk[i],  (1-kin[i]*(kin[i+1]-1)*kin[i+2]), alpha, rgsw_key);
	tGswSymEncryptInt(&bk2->bk[i+1],(1-(kin[i]-1)*kin[i+1]*kin[i+2]), alpha, rgsw_key);
	tGswSymEncryptInt(&bk3->bk[i],  (kin[i]*(kin[i+1]-1)*(kin[i+2]-1)), alpha, rgsw_key);
	tGswSymEncryptInt(&bk3->bk[i+1],((kin[i]-1)*kin[i+1]*(kin[i+2]-1)), alpha, rgsw_key);
	tGswSymEncryptInt(&bk4->bk[i],  ((kin[i]-1)*(kin[i+1]-1)*kin[i+2]), alpha, rgsw_key);
	tGswSymEncryptInt(&bk4->bk[i+1],(1-(kin[i]-1)*(kin[i+1]-1)*(kin[i+2]-1)), alpha, rgsw_key);	
    }
